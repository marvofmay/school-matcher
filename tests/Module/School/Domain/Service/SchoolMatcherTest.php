...
final class SchoolMatcherTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        $schools = [
            School::create(
                ...
                [SchoolName::create('LO Mickiewicza'), SchoolName::create('Mickiewicz'), SchoolName::create('I LO')],
                'Warszawa',
                'liceum'
            ),
            School::create(
                SchoolName::create('XIV Liceum Ogólnokształcące im. Stanisława Staszica'),
                [SchoolName::create('Staszic'), SchoolName::create('XIV LO'), SchoolName::create('14 LO')],
                'Warszawa',
                'liceum'
            ),
        ];
    ...
    public function testConstructorStoresSchools(): void
    {
        $matcher = new SchoolMatcher([]);
        $this->assertInstanceOf(SchoolMatcher::class, $matcher);
    }
    ...
    public function testExactNameMatch(): void
    {
        ...
        $this->assertSame('I Liceum Ogólnokształcące im. Adama Mickiewicza', $results[0]['school']);
    ...
    {
        $results = $this->matcher->match('Staszic', 'Kraków'); // żaden nie pasuje
        $this->assertSame([], $results);

        $results = $this->matcher->match('Staszic', 'Warszawa'); // pasuje
        $this->assertNotEmpty($results);
    }

    public function testTypeFilterExcludesOtherTypes(): void
    {
        $results = $this->matcher->match('Staszic', null, 'technikum'); // żaden nie pasuje
        $this->assertSame([], $results);

        $results = $this->matcher->match('Staszic', null, 'liceum'); // pasuje
        $this->assertNotEmpty($results);
    }

    public function testNoMatchReturnsEmptyArray(): void
    {
        $results = $this->matcher->match('Nieistniejąca Szkoła');
        $this->assertSame([], $results);
    }
